...
echo -e "\e[00;33m# Recommended: ./LinEnum.sh -k 'pass\|password' -r ReportName.txt -e /tmp/ -s -t \e[00m\n"

		echo "OPTIONS:"
		echo "-k	Enter keyword"
		echo "-e	Enter export location"
    echo "-s  Write software versions (only) to separate file: verinfo.txt"
...
if [ "$issue" ]; then
  echo -e "\e[00;31mEtc Issue information:\e[00m\n$issue"
  echo -e "\n"
  if [ "$verinfo" = "1" ]; then
...
if [ "$whoamicmd" ]; then
  echo -e "\e[00;31mCurrent user info:\e[00m\n$whoamicmd"
  echo -e "\n"
else
  :
fi
...
if [ "$usersfrompasswd" ]; then
  echo -e "\e[00;31mUser IDs only via /etc/passwd:\e[00m\n$usersfrompasswd"
  echo -e "\n"
else
  :
fi
...
if [ "$grpinfo" ]; then
  echo -e "\e[00;31mGroup memberships via id of f1 of etc/passwd:\e[00m\n$grpinfo"
...
if [ "$etcprofilecmd" ]; then
  echo -e "\e[00;31mETC Profile file may contain interesting information:\e[00m\n$etcprofilecmd"
  echo -e "\n"
else
  :
fi
...
sshfiles=`find / \( -name "id_dsa*" -o -name "id_rsa*" -o -name "identity*" -o -name "known_hosts" -o -name "authorized_hosts" -o -name "authorized_keys" \) -exec ls -la {} 2>/dev/null \;`
	if [ "$sshfiles" ]; then
		echo -e "\e[00;31mSSH keys/host information found in the following locations. If known_hosts, it may be crackable:\e[00m\n$sshfiles"
		echo -e "\n"
	else
		:
	fi
...
if [ "$bashversion" ]; then
  echo -e "\e[00;31mBash version:\e[00m\n$bashversion"
  echo -e "\n"
  if [ "$verinfo" = "1" ]; then
...
if [ "$shellshock" ]; then
  echo -e "\e[00;32mShellshock check : \e[00m\n$shellshock"
  echo -e "\n"
else
  :
fi
...
if [ "$ansiblecmd" ]; then
  echo -e "\e[00;31mAnsible automation software installed, it may be in use\e[00m\n$ansiblecmd"
  echo -e "\n"
else
  :
fi
...
if [ "$cfenginecmd" ]; then
  echo -e "\e[00;31mCFEngine automation software installed, it may be in use\e[00m\n$cfenginecmd"
  echo -e "\n"
else
  :
fi
...
if [ "$chefcmd" ]; then
  echo -e "\e[00;31mChef automation software installed, it may be in use\e[00m\n$chefcmd"
  echo -e "\n"
else
  :
fi
...
if [ "$puppetcmd" ]; then
  echo -e "\e[00;31mPuppet automation software installed, it may be in use\e[00m\n$puppetcmd"
  echo -e "\n"
else
  :
fi
...
if [ "$gpgfolder" ]; then
  echo -e "\e[00;31mgnupg folder found in a user's home dir. Check for lax permissions and accessible keys!:\e[00m\n$gpgfolder"
  echo -e "\n"
else
  :
fi
...
if [ "$keyword" = "" ]; then
  ...
  if [ -d "/var/www/" ]; then
    ...
    if [ "$wwwkey" ]; then
      echo -e "\e[00;31mFind keyword ($keyword) in /var/www files (output format filepath:identified line number where keyword appears):\e[00m\n$wwwkey"
      echo -e "\n"
    else
     echo -e "\e[00;31mFind keyword ($keyword) in /var/www files:\e[00m"
...
if [ "$keyword" = "" ]; then
  ...
  if [ -d "/home" ]; then
    ...
    if [ "$hiddenkey" ]; then
      echo -e "\e[00;31mFind keyword ($keyword) in /home/.* files (output format filepath:identified line number where keyword appears):\e[00m\n$hiddenkey"
      echo -e "\n"
    else
     echo -e "\e[00;31mFind keyword ($keyword) in /home/.* files:\e[00m"
